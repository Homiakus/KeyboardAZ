...
configurator_path = Path("go-app/configurator.go")
configurator = configurator_path.read_text(encoding="utf-8")
old_active = "\tactive := a.activeButtonsMask&(1<<uint(button)) != 0\n"
new_active = "\tactive := false\n\tif a.coreState != nil {\n\t\tactive = a.coreState.Snapshot().ActiveButtonsMask&(1<<uint(button)) != 0\n\t}\n"
if old_active not in configurator:
    raise SystemExit("configurator active button mirror reference missing")
configurator = configurator.replace(old_active, new_active, 1)
configurator_path.write_text(configurator, encoding="utf-8")

main_test_path = Path("go-app/main_test.go")
main_test_path.write_text(
    '''package main
