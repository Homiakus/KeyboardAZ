...
func TestButtonMessageEventIsLosslessAndCopiesButtons(t *testing.T) {
	...
	if event.Protocol != message.Protocol || event.Type != message.Type || event.Sequence != message.Sequence || event.Button != message.Button || event.Language != message.Language {
		t.Fatalf("event lost fields: %+v", event)
	}
	event.Buttons[0] = 1
	if message.Buttons[0] != 7 {
		t.Fatal("event shares mutable button slice with serial message")
	}
}
